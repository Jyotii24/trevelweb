        ...
        return `
            <div class="destination-card" id="dest-${dest.id}">
                ...
                <div class="destination-details">
                    ...
                    <p class="hotel-info">Activities: ${dest.activities.join(', ')}</p>
                </div>
...
function savePlace(id, heartBtn) {
    ...
    if (!savedPlaces.some(dest => dest.id === id)) {
...
// Function to render saved places
function renderSavedPlaces() {
    const container = document.getElementById('savedDestinationsContainer');
    container.innerHTML = savedPlaces.length === 0 
        ? '<p>No saved places yet.</p>' 
        : savedPlaces.map((dest, index) => `
...
// Function to toggle the visibility of saved places
function toggleSavedPlaces() {
    const savedContainer = document.getElementById('savedPlacesContainer');
    savedContainer.style.display = (savedContainer.style.display === 'none' || savedContainer.style.display === '') ? 'block' : 'none';
    renderSavedPlaces();
}
...
function saveToItinerary(id, heartBtn) {
    const selectedDestination = destinations.find(dest => dest.id === id);
    if (!itinerary.some(dest => dest.id === id)) {
...
function renderItinerary() {
    ...
    container.innerHTML = itinerary.length === 0 
        ? '<p>No destinations in your itinerary yet.</p>' 
        : itinerary.map(dest => `
...
function showModal(img, description, hotels) {
    ...
    document.getElementById('modalDescription').innerHTML = `<p>${description}</p><p>Hotels: ${hotels}</p>`;
}
